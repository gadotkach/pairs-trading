...
def main():
    ...
    print('=' * 70)
    print('STRATEGY 5: all pairs, summa {:,} rub.'.format(INITIAL_CAPITAL))
    print('=' * 70)
    ...
    with pd.ExcelWriter(OUTPUT_FILE, engine='openpyxl') as w:
        for period_name, df in results.items():
            sheet_name = '{}_Страт5'.format(period_name).replace(' ', '')
            df.to_excel(w, sheet_name=sheet_name[:31], index=False)

    print('=' * 70)
    print('Gotovo: {}'.format(OUTPUT_FILE))
    print('Listy: {}'.format(', '.join('{}_Страт5'.format(p) for p in results.keys())))
    print('=' * 70)
